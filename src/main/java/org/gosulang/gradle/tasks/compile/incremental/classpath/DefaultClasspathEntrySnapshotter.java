...
import org.gosulang.gradle.tasks.compile.incremental.deps.ClassDependentsAccumulator;
...
public class DefaultClasspathEntrySnapshotter {
  private static final Logger LOGGER = LoggerFactory.getLogger(org.gradle.api.internal.tasks.compile.incremental.classpath.DefaultClasspathEntrySnapshotter.class);
  ...
  private void visit(File classpathEntry, Map<String, HashCode> hashes, ClassDependentsAccumulator accumulator) {
    if (hasExtension(classpathEntry, ".jar")) {
      fileOperations.zipTree(classpathEntry).visit(new DefaultClasspathEntrySnapshotter.JarEntryVisitor(accumulator, hashes));
    }
    if (classpathEntry.isDirectory()) {
      fileOperations.fileTree(classpathEntry).visit(new DefaultClasspathEntrySnapshotter.DirectoryEntryVisitor(accumulator, hashes));
    }
  }
  ...
  private class JarEntryVisitor extends DefaultClasspathEntrySnapshotter.EntryVisitor {
  ...
  private class DirectoryEntryVisitor extends DefaultClasspathEntrySnapshotter.EntryVisitor {
