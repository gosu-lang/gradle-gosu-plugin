...
public class GosuConventionalSourceFileClassNameConverter {
  private final CompilationSourceDirs sourceDirs;

  public GosuConventionalSourceFileClassNameConverter(CompilationSourceDirs sourceDirs) {
    this.sourceDirs = sourceDirs;
  }

  public Collection<String> getClassNames(File sourceFile) {
    return sourceDirs.relativize(sourceFile)
        .map(relativePath -> Collections.singletonList(relativePath.replace('/', '.').replaceAll("\\.gsx?$", "")))
