...
public class ClasspathEntrySnapshot {
  ...
  public DependentsSet getAllClasses() {
    ...
    for (Map.Entry<String, HashCode> cls : getHashes().entrySet()) {
      ...
      DependentsSet dependents = getClassAnalysis().getRelevantDependents(className, Collections.emptySet());
  ...
  public Set<Integer> getRelevantConstants(ClasspathEntrySnapshot other, Set<String> affectedClasses) {
    Set<Integer> result = new HashSet<>();
    for (String affectedClass : affectedClasses) {
      Set<Integer> difference = new HashSet<>(other.getData().getClassAnalysis().getConstants(affectedClass));
